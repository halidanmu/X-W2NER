...
def process_bert(data, tokenizer, vocab):
    ...
    for index, instance in enumerate(data):
        ...
        #tokens = [tokenizer.tokenize(word, padding=True, truncation=True, max_length=510) for word in instance['sentence']]
        pieces = [piece for pieces in tokens for piece in pieces]
        #_bert_inputs = tokenizer.convert_tokens_to_ids(pieces)
        #_bert_inputs = tokenizer.encode(pieces, padding=True, truncation=True, max_length=510, is_split_into_words=True)
        _bert_inputs = tokenizer.encode(pieces, truncation=True, add_special_tokens=False, max_length=510, is_split_into_words=True)
...
def load_data_bert(config):
    ...
    tokenizer = AutoTokenizer.from_pretrained(config.bert_name, cache_dir="/root/autodl-nas/liuchang/huggingface/transformers")
