...
y_dev = []
y_dev2 = []
with open('/root/autodl-nas/liuchang/baseline/THUUMS/data/dev.trg', 'r', encoding='utf-8') as label_file:
    for i in label_file:
        labels = []
        for a in i.replace('\n', '').replace(' ', ''):
            labels.append(a)
        y_dev2.append(labels)
        y_dev.extend(labels)

with open('/root/autodl-nas/liuchang/baseline/THUUMS/data/test.trg', 'r', encoding='utf-8') as label_file:
    for i in label_file:
        labels = []
        for a in i.replace('\n', '').replace(' ', ''):
            labels.append(a)
        y_true2.append(labels)
    ...
    def eval(self, epoch, data_loader, is_test=False):
        ...
        with torch.no_grad():
            for i, data_batch in enumerate(data_loader):
                ...
                #ent_c, ent_p, ent_r, _, predicts_list, ent_set_list = utils_ms.decode(outputs.cpu().numpy(), entity_text,
                                                                                   #length.cpu().numpy(), predicts_list,
                                                                                   #ent_set_list)
                ent_c, ent_p, ent_r, decode_entities, predicts_list, ent_set_list = utils_ms.decode(outputs.cpu().numpy(), entity_text,
                                                                                   length.cpu().numpy(), predicts_list,
                                                                                   ent_set_list)########

                for ent_list in decode_entities:######
                    predict_label=[]
                    ent_list.sort()
                    for ent in ent_list:########
                        type = config.vocab.id_to_label(ent[1])########
                        predict_label.append(type)
                    y_pred.extend(predict_label)
                    y_pred2.append(predict_label)
        ...
        for label in predicts_list:
            per_list = []
            for element in label:
                per_list.append(element[1])
            predicts_list2.append(per_list)

        for label in ent_set_list:
            per_list = []
            for element in label:
                per_list.append(element[1])
            ent_set_list2.append(per_list)

        #print("###########"+"TEST_macro"+"###########") if is_test else print("###########"+"EVAL_macro"+"###########")

        #print(precision_recall_fscore_support(y_true if is_test else y_dev, y_pred, average='macro',labels=['b', 'm', 'e', 's']))
        #print(classification_report(y_true = y_true if is_test else y_dev,
                                    #y_pred = y_pred,
                                    #labels = ['b', 'm', 'e', 's'],
                                    #digits = 5))
        p0, r0, f10, _ = precision_recall_fscore_support(y_true if is_test else y_dev, y_pred, average='macro',labels=['b', 'm', 'e', 's'])

        '''
        #print(predict_list2)
        y_pred = []
        y_true = []
        for i in predicts_list2:
            y_pred.extend(i)
        for j in ent_set_list2:
            y_true.extend(j)
        #print(len(y_pred))
        #print(len(y_true))
        print("###########"+"TEST_macro"+"###########") if is_test else print("###########"+"EVAL_macro"+"###########")
        ...
        p, r, f1, _ = precision_recall_fscore_support(label_result.cpu().numpy(),
                                                      pred_result.cpu().numpy(),
                                                      average="macro")
        e_f1, e_p, e_r = utils_ms.cal_f1(total_ent_c, total_ent_p, total_ent_r)

        title = "EVAL" if not is_test else "TEST"
        #(label_result.cpu().numpy())
        ...
        table.add_row(["Entity"] + ["{:3.4f}".format(x) for x in [f10, p0, r0]])
    ...
    def predict(self, epoch, data_loader, data):
        ...
        with torch.no_grad():
            for data_batch in data_loader:
                ...
                i += config.batch_size
        #print(pred_result)
        p0, r0, f10, _ = precision_recall_fscore_support(y_true , y_pred, average='macro',
                                                         labels=['b', 'm', 'e', 's'])
        label_result = torch.cat(label_result)
        pred_result = torch.cat(pred_result)
...
if __name__ == '__main__':
    ...
    logger = utils_ms.get_logger(config.dataset)
    logger.info(config)
