...
def decode(outputs, entities, length, predicts_list, ent_set_list):
    ...
    for index, (instance, ent_set, l) in enumerate(zip(outputs, entities, length)):
        ...
        #predicts2 = [convert_index_to_text(x, ht_type_dict[(x[0], x[-1])]) for x in predicts]
        predicts = set([convert_index_to_text(x, ht_type_dict[(x[0], x[-1])]) for x in predicts])

        predicts_list.append([convert_text_to_index(x) for x in predicts])

        ent_set_list.append([convert_text_to_index(x) for x in ent_set])
        ...
        for x in predicts:
            if x in ent_set:
                ent_c += 1
    return ent_c, ent_p, ent_r, decode_entities, predicts_list, ent_set_list
